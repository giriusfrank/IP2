...
int main()
{
    ...
    log("\n[TEST] Bulk Insertions via operator+=");
    ...
    log("\n[TEST] Specific Searches via operator[]");
    ...
    log("\n[TEST] Multiple Deletions via operator-=");
    ...
    log("\n[TEST] Edit Operation via operator%=");
    EditOp ed = {70, 75};
    listA %= ed;
    log("Edit 70 to 75 | Status: " + std::string(listA[75] && !listA[70] ? "PASS" : "FAIL"));

    log("\n[TEST] Deep Copy & All 6 Logical Comparisons");
    ...
    log("\n[TEST] Memory Cleanup via operator!");
    !listB;
